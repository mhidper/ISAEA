...
def generar_excel_impactos(news_results, ficheros):
    ...
    para el Excel consolidado, tal como se solicitó (sin desgloses).
    """
    if not news_results:
        return pd.DataFrame()
        ...
        if st.session_state.get('estimar_dfmq', False):
            ...
            with download_container:
                ...
                for i, (name, result) in enumerate(news_results.items()):
                    ...
                    # Preparar datos detallados incluyendo revisión y novedades
                    impact_data = {
                    ...
                    # Añadir impacto por revisión y por novedades si están disponibles
                    if 'revision' in impact_df.columns:
                        impact_data['Impact Revision'] = impact_df['revision']
                    if 'news' in impact_df.columns:
                        impact_data['Impact News'] = impact_df['news']
                        
                    # Añadir previsiones si están disponibles
                    impact_data['Previous Forecast'] = impact_df.get('previous forecast', 'N/A')
                    impact_data['Revised Forecast'] = impact_df.get('revised forecast', 'N/A')
                    
                    detailed_df = pd.DataFrame(impact_data)
