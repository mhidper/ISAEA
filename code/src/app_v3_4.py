...
def generar_excel_impactos(news_results, ficheros):
    ...
    para el Excel consolidado, incluyendo novedades y revisiones de datos históricos.
    """
    if not news_results:
        return pd.DataFrame()
    ...
    for i in range(1, len(ficheros)):
        ...
        if vintage in news_results:
            ...
            for _, row in impact_df.iterrows():
                rows.append({
                    'Variable': sanitize_excel_string(row['updated variable']),
                    'Fecha': formatear_fecha_excel(row['update date']),
                    f'Impact Total ({key})': row['impact']
                })
            
            # Incluir impacto de revisiones de datos históricos si existe
            try:
                if hasattr(result, 'impacts') and 'impact of revisions' in result.impacts.columns:
                    revisions_impact = float(result.impacts['impact of revisions'].iloc[0])
                    if abs(revisions_impact) > 1e-5:
                        rows.append({
                            'Variable': 'Revisiones de datos históricos',
                            'Fecha': 'Revisiones',
                            f'Impact Total ({key})': revisions_impact
                        })
            except Exception:
                pass
...
if uploaded_files:
    ...
    if dfs_m:
        ...
        if st.session_state.get('estimar_dfmq', False):
            ...
            forecast_mensuales_f = forecast_mensuales[forecast_mensuales.index >= pd.to_datetime('2022-01-01')]
            forecast_trimestrales_f = forecast_trimestrales[forecast_trimestrales.index >= pd.to_datetime('2022-01-01')]
            ...
            with download_container:
                ...
                for i, (name, result) in enumerate(news_results.items()):
                    ...
                    # Añadir impacto de revisiones históricas si existe
                    try:
                        if hasattr(result, 'impacts') and 'impact of revisions' in result.impacts.columns:
                            revisions_impact = float(result.impacts['impact of revisions'].iloc[0])
                            if abs(revisions_impact) > 1e-5:
                                rev_row = pd.DataFrame([{
                                    'Comparison': key,
                                    'Updated Variable': 'Revisiones de datos históricos',
                                    'Update Date': 'Revisiones',
                                    'Impact Total': revisions_impact,
                                    'News (Surprise)': 0.0,
                                    'Previous Forecast': 0.0,
                                    'Revised Forecast': 0.0
                                }])
                                detailed_df = pd.concat([detailed_df, rev_row], ignore_index=True)
                    except Exception:
                        pass
            ...
            if news_results:
                try:
                    ...
                    primer_news = news_results[vintages_list[0]]
                    
                    # Primera estimación (del vintage base, evaluada para impact_date)
                    try:
                        if hasattr(primer_news, 'impacts') and 'estimate (prev)' in primer_news.impacts.columns:
                            primera_estimacion = float(primer_news.impacts['estimate (prev)'].iloc[0])
                        else:
                            ...
                            if len(summary_tables) > 1:
                                ...
                                if 'estimate (prev)' in summary_df.columns:
                                    ...
                                    primera_estimacion = float(vintage_results[primer_vintage].get_prediction(start='2006-01', end=impact_date).predicted_mean['pib'].resample('Q').mean().iloc[-1])
                            else:
                                primera_estimacion = float(vintage_results[primer_vintage].get_prediction(start='2006-01', end=impact_date).predicted_mean['pib'].resample('Q').mean().iloc[-1])
                    except Exception:
                        primera_estimacion = float(vintage_results[primer_vintage].get_prediction(start='2006-01', end=impact_date).predicted_mean['pib'].resample('Q').mean().iloc[-1])
                    ...
                    # Estimaciones de cada vintage posterior (desde news para impact_date)
                    for vintage in vintages_list:
                        try:
                            if hasattr(news_results[vintage], 'impacts') and 'estimate (new)' in news_results[vintage].impacts.columns:
                                nueva_estimacion = float(news_results[vintage].impacts['estimate (new)'].iloc[0])
                            else:
                                ...
                                if len(summary_tables) > 1:
                                    ...
                                    if 'estimate (new)' in summary_df.columns:
                                        nueva_estimacion = float(summary_df['estimate (new)'].iloc[0])
                                    else:
                                        nueva_estimacion = float(news_results[vintage].total_impacts.iloc[-1] + point_estimates[-1]['point estimate'])
                                else:
                                    nueva_estimacion = float(news_results[vintage].total_impacts.iloc[-1] + point_estimates[-1]['point estimate'])
                        except Exception:
                            nueva_estimacion = float(point_estimates[-1]['point estimate'] + news_results[vintage].details_by_impact['impact'].sum())
                    ...
                    for vintage in vintages_list:
                        ...
                        imp_dict = impacts.to_dict()
                        
                        # Añadir impacto de revisiones históricas si existe
                        try:
                            if hasattr(news_results[vintage], 'impacts') and 'impact of revisions' in news_results[vintage].impacts.columns:
                                rev_impact = float(news_results[vintage].impacts['impact of revisions'].iloc[0])
                                if abs(rev_impact) > 1e-5:
                                    imp_dict['Revisiones de datos'] = rev_impact
                        except Exception:
                            pass
                            
                        impacts_by_vintage[vintage] = imp_dict
